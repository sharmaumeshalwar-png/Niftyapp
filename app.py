...
st.markdown("""
    <div class="title-block">
        <h1>🎯 Nifty 50 Pure Cascade (Strict 1 July 2025 - 1 Hour)</h1>
        <p><b>Interval:</b> 1 Hour (1h) | <b>Start Filter:</b> Strict 01 Jul 2025<br>
        <b>Column L Rule:</b> (% Change of H from F) - (% Change of F from C)<br>
        <b>Column M Rule:</b> Delta of Column L i.e. <code>L[i] - L[i-1]</code></p>
    </div>
""", unsafe_allow_html=True)

# 2. DATA PIPELINE (STRICT START DATE SET TO 1 JULY 2025)
@st.cache_data(ttl=300)
def load_pure_data():
    # Strict start date filter direct download me lagaya h
    df_raw = yf.download(tickers="^NSEI", start="2025-07-01", interval="1h")
    if df_raw.empty:
        return pd.DataFrame()
...
if not df.empty:
    ...
    # 1 Hour candles ke liye clear format
    ...
    # 5. COLUMN M VALIDATOR (M = L[i] - L[i-1])
