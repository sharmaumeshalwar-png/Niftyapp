...
st.write("🎯 **Untouched Zone Memory Setup:** 2 Years Data + Strict 50:50 Split + Persistent Supply/Demand Memory Engine")
...
with st.spinner("Extracting Clean 2-Year Matrices with Untouched Zone Persistence..."):
    raw_df = yf.download("BTC-USD", period="730d", interval="1h", multi_level_index=False)
    
    if raw_df.empty:
        st.error("🚨 Market API Endpoint Error. Please reload the dashboard.")
        st.stop()
