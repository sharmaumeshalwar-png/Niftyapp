...
st.write("🎯 **Pure Direct Signals:** Magic Trend Intensity (Hurst + ATR-Momentum Fusion) Engine")
...
# Volatility-Normalized ATR-Weighted Momentum (Direct ATR division, ATR_Kalman removed)
df['ATR_Weighted_Momentum'] = df['Weighted_Momentum'] / (df['ATR'] + 1e-8)

# 🔮 NAYA MAGIC COLUMN: Hurst + ATR-Weighted Momentum Hybrid Theory
# Hurst exponent momentum vectors ko dynamically scale/amplify karta hai
df['Magic_Trend_Intensity'] = df['ATR_Weighted_Momentum'] * (df['Hurst'] * 2.0)

# Split Engine 50:50
split_idx = int(len(df) * 0.50)
df_predict = df.iloc[split_idx:].copy()

# 🤖 PURE SIGNAL ENGINE LOGIC (Driven by Magic Trend Intensity)
...
# 🚀 HIGH CONTRAST PROBABILITIES GENERATION BASED ON MAGIC INTENSITY
prob_up = []
prob_down = []
for idx in range(len(df_predict)):
    sig = signal_log[idx]
    ...
    # Mathematical Sigmoid-scaling based on Magic Matrix Strength
    intensity = min(0.49, abs(magic_val) * 0.15) 
    
    if sig == "🟢 BUY":
        p_up = round(0.50 + intensity, 2)
        if p_up < 0.55: p_up = 0.58 # baseline adjustment
        prob_up.append(p_up)
...
# Format Layout Columns Matrix (ATR_Kalman removed, Magic Column added)
