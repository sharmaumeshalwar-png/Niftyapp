...
# 🔥 CRITICAL FIX 1: Split data FIRST. 
# Pure math functions is isolated slice ke baahar ka data nahi dekh sakti.
...
# 🔥 CRITICAL FIX 2: Kalman and Indicators run ONLY on the isolated slice
df_predict['Kalman_Baseline'] = apply_kalman_filter_custom(close_arr, initial_p=50.0, q_val=0.0005, r_val=0.2)

# 🔥 CRITICAL FIX 3: Removed .bfill() completely to eliminate Lookahead Bias on ATR
...
# Hurst Memory Calculation on Isolated Data
...
# Securely drop initial NaN rows generated by rolling windows (No bfill cheat allowed)
df_predict.dropna(subset=['ATR', 'Hurst'], inplace=True)

# 🤖 PURE SIGNAL ENGINE LOGIC (Same core logic as your original script)
...
# 🚀 HIGH CONTRAST PROBABILITIES GENERATION (Same original logic)
prob_up = []
prob_down = []
for idx in range(len(df_predict)):
    sig = signal_log[idx]
