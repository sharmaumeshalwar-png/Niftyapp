...
st.title("⚡ Bitcoin (BTC-USD) 2-Year Pure Kinematic Engine + Acceleration Filter")
st.write(
    "🎯 **1-Hour Timeframe Engine:** 2-Year Full History | Pure HAM Kinematics "
    "Matrix | Velocity (Δ1) & Acceleration (Δ2) Auto-Flip Filter | IST Locked [Strict Zero Leakage & Continuous Warmup]"
)
...
# STEP 6 & 7: VELOCITY, ACCELERATION AND FLIP FILTER ENGINE
def compute_flip_filter(df_in):
    df = df_in.copy()

    # 1. HAM Velocity (1st Derivative of HAM Diff)
    df["HAM_Velocity"] = df["HAM_Diff"].diff()

    # 2. HAM Acceleration (2nd Derivative of HAM Diff)
    df["HAM_Acceleration"] = df["HAM_Velocity"].diff()

    # 3. Pure Acceleration Flip Detection Rules
    conditions = [
        # Real Bearish Flip: Diff dropping, Velocity < 0, AND Acceleration < 0
        (df["HAM_Diff"] < df["HAM_Diff"].shift(1))
        & (df["HAM_Velocity"] < 0)
        & (df["HAM_Acceleration"] < 0),
    ...
    df["Flip_Status"] = np.select(
        conditions, choices, default="🟡 STABLE / CONTINUATION"
    )
    return df
...
# --- ADD VELOCITY, ACCELERATION & FLIP STATUS ---
