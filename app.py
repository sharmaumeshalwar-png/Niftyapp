...
# Page Configuration
st.set_page_config(page_title="Nifty Total Market Kalman Engine", layout="wide")
st.title("📊 Nifty Total Market Live 1-Hour Double Kalman [Pure Native Volatility Engine]")
st.write("🎯 **Aapki Custom Setting:** Nifty Total Index Data + Native Volatility Index Sync + Fixed 25-Candle Window + 3-Regime Dynamic Barriers + Double Kalman Smooth Momentum (P=0.50)")
...
def apply_kalman_filter_adaptive(data_array, regimes_array, initial_p=50.0):
    ...
    for z, regime in zip(data_array, regimes_array):
        # Strictly adaptive noise selection based on Index Native Volatility regime
        if regime == 2:    # Hyper Liquid Momentum Expansion (High Volatility Panic)
            q = 0.005      # Ultra-responsive tracking for institutional waves
            r = 0.05       
        elif regime == 0:  # Tight Volume Compression (Low Volatility Sideways)
            q = 0.0005     # Eliminate minor noise filters
            r = 0.2        
...
with st.spinner("Building Native Nifty Volatility & Pricing Matrices..."):
    # Download raw Nifty Total Market / Nifty 50 proxy data (2 Years Window)
    ...
    # =====================================================================
    # MATHEMATICAL ENGINE: PURE NATIVE INDEX VOLATILITY CALCULATION (25-Candle Structure)
    # =====================================================================
    ...
    df['Index_ATR_25'] = df['True_Range'].rolling(window=25).mean()
    ...
    df['Index_Std_25'] = df['Log_Ret'].rolling(window=25).std() * np.sqrt(24 * 365) * 100 # Annualized Intraday Scale
    
    # Step D: Synthetic Pure Index VIX Vector Composition
    df['Index_VIX'] = (df['Index_ATR_25'] / df['a_Close'] * 1000) + (df['Index_Std_25'] * 0.1)
    df['Index_VIX'] = df['Index_VIX'].ffill().fillna(15.0)

    # Strictly Fixed 25-Candle Rolling Feature for the Volatility Matrix
    df['VIX_Rolling_25'] = df['Index_VIX'].rolling(window=25).mean().ffill().fillna(15.0)

    # STRICT NATIVE INDEX VOLATILITY 3-REGIME MAPPING BASED ON PERCENTILES
    vix_low = df['Index_VIX'].quantile(0.30)
    vix_high = df['Index_VIX'].quantile(0.85)
    
    df['VIX_Regime'] = np.where(df['Index_VIX'] < vix_low, 0, np.where(df['Index_VIX'] <= vix_high, 1, 2))
...
if len(X_predict) == 0:
    ...
    for i in range(len(prob_ups)):
        ...
        # INDEX DYNAMIC OVERRIDES FOR ACTIVATION BARRIERS
        if current_regime == 2:      # High Volatility Institutional Expansion
            barrier = 0.62           # Strict checkpoint to bypass momentum traps
        elif current_regime == 0:    # Low Volatility Sidechain Compression
            barrier = 0.52           # High sensitivity tracking for early breakouts
    ...
    df_predict['Index_VIX'] = df_predict['Index_VIX'].round(2)
    clean_display_cols = ['Index_VIX', 'VIX_Rolling_25', 'VIX_Regime', 'a_Close', 'b_Kalman_Price', 'Prob_Up', 'Prob_Down', 'Accumulator_Score', 'Weighted_Momentum', 'd_ML_Signal']
    ...
    st.subheader(f"📋 Live 1-Hour Nifty Total Index Matrix (Adaptive Volatility Architecture)")
