...
if not df.empty:
    ...
    # Custom Visual Grid Theme Engine (Red for Fear Spike, Green for Calm Market)
    ...
    # Frame Grid Structure with High Decimal Formats for VIX Tracking
    st.dataframe(show_df.style.format({
        'Column A': '{:.2f}', 'Column B': '{:.4f}', 'Column C': '{:.4f}'
    }).map(color_vix_grid, subset=['Column E']), use_container_width=True)
else:
    st.error("India VIX dynamic dataset fetch load block ho gaya hai.")
