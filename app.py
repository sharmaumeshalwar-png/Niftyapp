...
st.markdown("""
    <div class="discovery-block">
        <h1>🌌 The Coppock & Column C Discovery Engine (Perfect Framework)</h1>
...
# STEP 3: ANTI-LEAK & ANTI-BLOCK INGESTION PIPELINE
if len(st.session_state.coppock_2026_perfect_db) == 0 or run_sync:
    with st.spinner("Compiling High-Fidelity Coppock Target Matrix..."):
        try:
            # Requesting 2y hourly boundary limit to cover strict timeline coordinates
            raw_feed = yf.download(tickers=ticker, interval="1h", period="2y", progress=False)
            using_fallback = False
            
            # Automated Fallback if Cloud IP is rate-limited/blocked
            ...
            # Dynamic ongoing candle drop logic safely
            if len(raw_feed) > 1:
            ...
            if total_elements > 0:
                ...
                col_c = np.zeros(total_elements, dtype=float)  # Column C (Delta Variance)
                coppock_curve = np.zeros(total_elements, dtype=float) # Coppock Curve Array
                time_list = ["" for _ in range(total_elements)]

                # STEP 5: PRE-COMPUTATION ROLLING WINDOW CALCULATIONS (Anti-Leak Math)
                # Pure mathematical calculations run globally before slicing to avoid edge-leak errors
                raw_series = pd.Series(col_a)
                
                # Column C Processing Loop
                col_c_series = raw_series - raw_series.shift(int(delta_lookback))
                col_c = col_c_series.fillna(0.0).values
                
                # Coppock Base Engine Vectors
                ...
                # Sync timestamps to matching array slots
                ...
                # Mapping rules: >= 2.5 (Extreme Overbought) and <= -2.5 (Extreme Oversold)
                excel_zones = []
                ...
                # Strict 2-Year Date Boundary Lock Filter
                ...
                # Crossover Signals applied post-mask to prevent cross-leak
...
if not output_matrix.empty:
    ...
    # Advanced Candlestick Chart Execution
    fig = go.Figure()
    ...
    # Plotting Bullish Hints (Long Entries)
    ...
    # Plotting Bearish Hints (Short Entries)
    ...
    # Displaying clean rows with the target columns
    st.dataframe(
