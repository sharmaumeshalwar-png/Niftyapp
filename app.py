...
st.title("Nifty Super-Responsive Filter (Fast 0.99 & Slow 0.005)")

st.write("Fixed Architecture: Fast Q=0.99 aur Slow R=0.005 Active Hai...")
...
if data.empty:
    ...
    # 3. FAST KALMAN (Fixed Q = 0.99 - Hyper Sensitive)
    b_fast = np.zeros(num_steps)
    x_fast = a_adjusted[0]
    P_f, Q_f, R_f = 1.0, 0.99, 1.0
    ...
    # 4. SLOW KALMAN (Fixed R = 0.005 - Ultra Low Noise Filter)
    b_slow = np.zeros(num_steps)
    x_slow = a_adjusted[0]
    P_s, Q_s, R_s = 1.0, 0.01, 0.005
    for t in range(num_steps):
    ...
    # Kyunki dono lines ab bhot close chalengi, isliye barrier ko 4.0 par optimize kiya hai
    signals = []
    ...
    df_table = pd.DataFrame({
        ...
        'b_fast (Q=0.99)': np.round(b_fast_real, 2),
        'b_slow (R=0.005)': np.round(b_slow_real, 2),
    ...
    st.success("System updated to Hyper-Responsive Configuration (0.99 & 0.005)!")
