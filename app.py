...
st.caption(
    "100% Leak-Free Grid with Dynamic Volatility-Adaptive Kalman Hurst Exponent & Market ER"
)
...
def apply_dynamic_kalman_hurst(hurst_series, volatility_ratio_series):
    ...
    for z, vol_ratio in zip(hurst_series, volatility_ratio_series):
        ...
        # Dynamic Scaling: Volatility spike = Fast Response, Low Volatility = High Smoothing
        # vol_ratio > 1.0 means higher than normal volatility
        q_dynamic = base_q * np.clip(vol_ratio, 0.5, 3.0)
        r_dynamic = base_r / np.clip(vol_ratio, 0.5, 3.0)

        # Kalman Engine Update
        p = p + q_dynamic
        k = p / (p + r_dynamic)
        x = x + k * (z - x)
        p = (1 - k) * p
...
def calculate_rolling_hurst_leak_free(df_ha, window=30):
    ...
    # Calculate Volatility Ratio (Current ATR vs 20-period Moving ATR) for Dynamic Scaling
    ...
    # Applying High Dynamic Adaptive Kalman Filter directly on Hurst Exponent
...
def compute_ha_ham_features(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    # Dynamic Adaptive Kalman-Filtered Hurst
...
for i in range(2, n):
    ...
    # Enhanced Chop Detection with Dynamic Adaptive Kalman Hurst
...
st.subheader("📋 Nifty 50 Timeline with Dynamic Adaptive Kalman Anti-Chop")
