...
st.title("Nifty & India VIX Enhanced Gap-Engine Dashboard")

# GAP ENGINE CONFIGURATION - Isko aap aur badha sakte hain (e.g., 2.0, 2.5)
gap_multiplier = 1.5 

st.write(f"Active Framework: Gap Multiplier Boosted at {gap_multiplier}x | Q=0.50 | Aggressive Momentum Hints...")
...
if combined_data is None or len(combined_data) == 0:
    ...
    # 2. ENHANCED GAP DE-TRENDING ENGINE (With Multiplier Boost)
    ...
    for t in range(1, num_steps):
        if parsed_dates[t] != parsed_dates[t-1]:
            ...
            if abs(gap) > 5.0:  
                # Gap impact ko multiplier se multiply karke badha diya gaya hai
                cumulative_gap += (gap * gap_multiplier)
    ...
    # Re-applying cumulative gap to align with mathematical signals
    ...
    # 6. BOOSTED TREND SIGNALS (Only Red and Green Blocks)
    nifty_signals = []
    ...
    st.success("Gap-Engine successfully activated with Multiplier boost! High-accuracy momentum filtering live.")
