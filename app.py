...
# Page Configuration
st.set_page_config(page_title="BTC Pure Signal Engine", layout="wide")
st.title("⚡ Bitcoin (BTC-USD) Pure Action Signal Engine")
st.write("🎯 **Pure Direct Signals:** Auto-Resolving Choppy Zones into Pure Action Triggers")

# =====================================================================
# MATHEMATICAL ENGINES (Kalman, Hurst, and ATR Matrix)
# =====================================================================
...
atr_val = df_predict['ATR'].to_numpy()
...
for idx in range(len(df_predict)):
    ...
    # 2. Consolidation & Micro Dynamic Resolution Zone
    else:
        # Resolve inner oscillations using mathematical mean deviation bounds
        deviation = raw_close[idx] - kalman_line[idx]
        if deviation >= 0:
            # If price trades upper half of the channel, build up momentum
            signal_log.append("🟢 BUY")
        else:
            # If price trades lower half of the channel, breakdown distribution
            signal_log.append("🔴 SELL")
...
for idx in range(len(df_predict)):
    ...
    if sig == "🟢 BUY":
        # High contrast amplification if the mathematical trend is highly persistent
