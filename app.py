...
# 1. Loading data safely with a secure 720 days limit to prevent yfinance empty blocks
@st.cache_data(ttl=300)
def load_auto_frozen_data_fixed():
    ticker = "BTC-USD"
    today_date = datetime.now().date()
    
    # 720 days is highly stable for hourly (1h) fetch on yfinance API
    start_date = today_date - timedelta(days=720)  
    
    try:
        # Download data securely
        df = yf.download(ticker, start=start_date, end=datetime.now(), interval="1h")
        
        # If dataset is empty, return an empty dataframe immediately
        if df.empty:
            return pd.DataFrame()
        ...
        # Dynamic Column Finder for Date/Time
        ...
        st.error(f"yfinance Download Error: {str(e)}")
        return pd.DataFrame()
...
try:
    ...
    if raw_data.empty:
        st.error("❌ Yahoo Finance returned NO data. Server is busy or time range is blocked. Please wait a few seconds and try refreshing the browser.")
    else:
        ...
        if len(range_closes) == 0:
            st.warning("⚠️ No range bars generated yet for this period.")
        else:
