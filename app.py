...
# -----------------------------------------------------------------
# 🛡️ SYSTEM DATA INGESTION (2-Year / 730D 1H Setup)
# -----------------------------------------------------------------
@st.cache_data(ttl=120)
def fetch_2y_btc_data():
    df_fetched = yf.download(tickers="BTC-USD", period="730d", interval="1h")
    ...
    # 🔒 STRICT LEAKAGE PREVENTION: Drop incomplete/running live candle!
...
# Strict 50:50 split matrix execution over historical backbuffer
split_idx = int(len(df) * 0.50)
df_predict = df.iloc[split_idx:].copy()

st.success(f"🟢 **Synced & Secured {len(df_predict)} IST Locked Bitcoin Candles (Zero Leakage Engine Active)!**")
...
col3.metric("Normal HAM", f"{latest_candle['HAM_Normal']}")
col4.metric("HA HAM", f"{latest_candle['HAM_HeikinAshi']}")
...
st.dataframe(
    display_df.style.background_gradient(
        subset=['HAM_Normal', 'HAM_HeikinAshi'], 
        cmap='coolwarm'
    ),
